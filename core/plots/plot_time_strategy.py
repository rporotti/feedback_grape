...
from core.create_state import create_batched_tensor_state
...
def plot(folder, steps, seed, name, n_average, filename=None, gamma=None, rho_target=None, cavity_state=None, just_one=False, plot_control=True, fig=None, gs=None, save=True, return_info=False, index=False, fake_measure=False, N_control_steps=1, N_cavity=10, rho_init=None, batch_size=None, timesteps=None, control=True, feedback=True, type_input="measure", compl_controls=False, epsilon=1E-10, lim_y=False, mode="network", new_method=False):
            ...
            if N_control_steps==1:
                network = tf.keras.Sequential(
                    [tf.keras.layers.Flatten(),
                     ...
                     tf.keras.layers.Dense(output_neurons, dtype="float64", bias_initializer=tf.keras.initializers.Constant(np.pi))]
                )
            else:
                network = tf.keras.Sequential(
                    [tf.keras.layers.Flatten(),
                     tf.keras.layers.Dense(30, input_shape=input_shape, activation='tanh', dtype="float64"),
                     tf.keras.layers.Dense(30, activation='tanh', dtype="float64"),
                     tf.keras.layers.Dense(output_neurons*N_control_steps, dtype="float64", bias_initializer=tf.keras.initializers.Constant(np.pi)),
        ...
        if type_input == "measure":
            ...
            if N_control_steps==1:
                network = tf.keras.Sequential(
                        [
                            tf.keras.layers.GRU(30, batch_input_shape=(batch_size, 1, 1), 
                                             stateful=True),
                            tf.keras.layers.Dense(output_neurons, dtype="float64", bias_initializer=tf.keras.initializers.Constant(np.pi/2))]
                        )
                ...
                [
                    tf.keras.layers.GRU(30, batch_input_shape=(batch_size, 1, 1), 
                                     stateful=True),
                    tf.keras.layers.Dense(output_neurons*N_control_steps, dtype="float64", bias_initializer=tf.keras.initializers.Constant(np.pi/2)),
                    tf.keras.layers.Reshape((N_control_steps, output_neurons))
...
#         info["fidelities"] = info["fidelities"][indices]
    ...
    #     info["controls"][:, :, 3] = (info["controls"][:, :, 3]/np.pi)%1
        ...
        n_av[0] = n_average
        im = ax[0].imshow(probabilities.T, interpolation=None, origin="lower", aspect="auto", cmap=my_cmap, norm=matplotlib.colors.LogNorm(vmin=lim, vmax=1), rasterized=True)
        ax[1].imshow(qubit_av.T, interpolation=None, origin="lower", aspect="auto", cmap=my_cmap, norm=matplotlib.colors.LogNorm(vmin=lim, vmax=1), rasterized=True)
...
#         ax[2].vlines(1, 0, (info["controls"][0, traj, 3]/np.pi), lw=lw, color=colors[3])
    ...
    #                       ncol=4, fancybox=True, shadow=True, columnspacing=1.0, handletextpad=0.3)
        ...
        if not just_one:
            ax[0].set_title(f'Fidelity: {np.round(info["fidelities"][-1][traj], 3)}, prob: {np.round(np.exp(info["log_probs"][-1][traj]), 3)}')
...
#             ax[i].grid(axis='y')
        ...
        ax[0].set(yticks=(np.arange(0, lim_y+1,1)), yticklabels=[r"$|{{{}}}\rangle_c$".format(x) for x in np.arange(0, lim_y+1, 1)],
                 xticks=np.arange(0, t_max, 1)-1/2, xticklabels=[int(idx/3) if x%3==0 else "" for idx, x in enumerate(np.arange(t_max))]
                 )
        if lim_y:
...
#                                  horizontalalignment='center', verticalalignment="center", zorder=50, clip_on=False)
        ...
        ax[4+shift].stairs(fidelity, np.arange(len(fidelity)+1)-1/2, lw=lw, color=colors[0], baseline=None, label=r"$\mathrm{Fidelity}$")
        ax[4+shift].stairs(purity, np.arange(len(purity)+1)-1/2, lw=lw, color=colors[1], baseline=None,label=r"$\mathrm{Purity}$")
#         if gamma==0.0:
...
#             ax[4+shift].text(5, fidelity[7]*1.05, r"$\mathrm{Fidelity}$", va="bottom", color=colors[0])
        ...
        if save:
            fig.savefig("pdfs/"+"{:04d}".format(idx)+".pdf", format="pdf", dpi=300, bbox_inches='tight', pad_inches=0)
        if just_one:
            fig.show()
        else:
            plt.close('all')
