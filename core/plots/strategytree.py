...
#                             zorder=10))
            self.ax.scatter(x,y,c=self.state_circle_fill_color,edgecolor=self.state_circle_color,s=self.measure_circle_radius, zorder=20)
            if measure:
...
#                             zorder=10))
            self.ax.scatter(x,y,c=self.measure_circle_fill_color,edgecolor=self.measure_circle_fill_color,s=self.node_circle_radius)
...
#                                  horizontalalignment='center', verticalalignment="center", zorder=10)
    ...
    def load_info_and_plot(self, folder, steps, seed, name, n_average, how_many=False, steps_to_plot=3, save=False, mode="network", rho_target=None, filename=None, N_cavity=None):
        if N_cavity is None:
            N_cavity = 10
        else:
            N_cavity = N_cavity
    ...
    def plot(self,measure_choices,control_choices,measurement_results, fidelities=None,spacing="double",
        ...
        for idx in range(batchsize):
            self.plot_path(measure_choices[idx],control_choices[idx],measurement_results[idx], fidelities[idx], goal=goal, spacing=spacing)
