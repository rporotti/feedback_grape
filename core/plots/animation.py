...
def animation(info, substeps=5, N_wigner=500, length=None, figsize=(8, 3)):
    ...
    for i in range(len(rhos)):
        ...
        if np.any(rho != rhos[i - 1]):
            ...
            rho_part_cav = tf.linalg.einsum("ikjk->ij", tf.reshape(rho, (
            int(rho.shape[1] / 2), 2, int(rho.shape[1] / 2), 2))).numpy()
            rho_part_qubit = tf.linalg.diag_part(tf.linalg.einsum("kikj->ij", tf.reshape(rho, (
            int(rho.shape[1] / 2), 2, int(rho.shape[1] / 2), 2)))).numpy()
